...
The `web_fetch` implementation is an anonymous public HTTP(S) fetch provider, `http`. It fetches bytes from a concrete URL, restricts destinations to public networks, applies the transport controls below, decodes textual content, and returns only the minimal model-useful result: final URL, status code, body, and truncation. It carries no browser cookies, editor credentials, git credentials, internal auth tokens, or implicit access to private services. [Public-network address validation and connection pinning](../bug-fix/2026-08-28-web-fetch-public-network-pinning.md) are part of this provider rather than a prompt or deployment assumption.
...
- Localhost aliases and non-public IPv4/IPv6 literals are rejected, including canonicalized alternative IPv4 forms, IPv4-mapped IPv6, transition prefixes, and private, loopback, link-local, shared, documentation, benchmark, multicast, or reserved ranges.
- Every hostname is resolved before connection; a mixed public/private answer is rejected, and a request-scoped Undici dispatcher can connect only to the validated answer set.
...
- Only same-origin redirects are followed automatically; a cross-origin redirect fails with `WEB_REDIRECT_BLOCKED`, requiring a fresh tool call and therefore a fresh provider/permission decision. Each allowed same-origin hop receives fresh DNS validation and a fresh dispatcher. (Claude Code's WebFetch uses this same cross-origin model — it returns the redirect target to the model for a fresh call.)
...
**Fetch is a network boundary, not just a read-only tool.** `web_fetch` can exfiltrate data through user-supplied public URLs, so URL arguments remain subject to normal tool-permission policy. The provider denies internal destinations without a bypass; an explicitly permissioned intranet connector is a separate capability rather than an exception in this anonymous provider.
...
- [One-shot outbound consent](../feature/2026-09-04-web-tool-outbound-consent.md) is enforced by the native tools when configured. Network-wide enforcement and persistent destination grants remain deferred.
...
- How should network-wide policy extend the [native tool consent](../feature/2026-09-04-web-tool-outbound-consent.md) to direct service callers and subprocesses?
