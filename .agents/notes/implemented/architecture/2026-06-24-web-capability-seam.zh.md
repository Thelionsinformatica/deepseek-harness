...
`web_fetch` 的实现是一个匿名公开 HTTP(S) fetch 提供方 `http`。它从具体 URL 获取字节，将目的地限制在公开网络，应用下述传输控制，解码文本内容，并仅返回最小的模型可用结果：最终 URL、状态码、正文和截断标志。它不携带浏览器 cookie、编辑器凭证、git 凭证、内部认证令牌，也不隐式访问私有服务。[公开网络地址验证与连接固定](../bug-fix/2026-08-28-web-fetch-public-network-pinning.zh.md)属于该提供方，而不是提示词或部署假设。
...
- 拒绝 localhost 别名和非公开 IPv4/IPv6 字面量，包括规范化后的替代 IPv4 形式、IPv4 映射 IPv6、过渡前缀，以及私有、回环、链路本地、共享、文档、基准测试、多播或保留地址范围。
- 每次连接前解析主机名；若 DNS 答案同时包含公开与私有地址则整体拒绝，请求级 Undici dispatcher 只能连接到验证后的地址集合。
- 强制执行最大 URL 长度、响应字节上限、解码正文字符上限、超时和重定向跳数上限。
- Abort 信号传播到网络获取和高开销解码。
- 仅自动跟随同源重定向；跨源重定向以 `WEB_REDIRECT_BLOCKED` 失败，要求一次新的工具调用，从而触发新的提供方/权限决策。每个允许的同源跳都使用新的 DNS 验证和新的 dispatcher。（Claude Code 的 WebFetch 使用同样的跨源模型：它将重定向目标返回给模型以发起新调用。）
...
**Fetch 是网络边界，不仅仅是只读工具。** `web_fetch` 仍可通过用户提供的公开 URL 外泄数据，因此 URL 参数继续受正常工具权限策略约束。提供方无旁路地拒绝内部目的地；若需访问 intranet，应使用独立且明确授权的连接器，而不是给这个匿名提供方增加例外。
...
- 原生工具在配置启用时强制执行[单次对外访问批准](../feature/2026-09-04-web-tool-outbound-consent.zh.md)。覆盖所有网络的约束与持久化目标授权仍暂缓。
...
- 覆盖所有网络的策略如何将[原生工具批准](../feature/2026-09-04-web-tool-outbound-consent.zh.md)扩展到直接服务调用方与子进程？
