...
The gate did keep one thing off the wire, and this note states it plainly because the decision has to survive the accurate version: a registered namespace the list did not name never had its resolved, `base`, or `user` values reach the browser at all. The separately governed Plugin Center is not a substitute — `PluginInventoryEntry` carries Loader identity, runtime state, safe inferred operational metadata, and only a deployment-audited process-local control; it never carries a stored setting value, raw Loader configuration, provenance, source paths, credentials, cookies, or tokens.
