...
这道门确实挡住了一样东西，本 note 如实写出，因为这个决策必须在准确版本下也站得住：不在名单上的已注册命名空间，其 resolved、`base` 与 `user` 值根本不会抵达浏览器。受到单独治理的插件中心不能替代它——`PluginInventoryEntry` 携带 Loader 标识、运行时状态、安全推断的运行元数据以及仅限部署审计的进程内控件；它绝不携带已存设置值、原始 Loader 配置、来源、源路径、凭据、cookie 或 token。
