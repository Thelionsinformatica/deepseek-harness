...
### Completion evidence and independent review

`completionRequiresCompletedTodos` defaults to false for composition compatibility. When enabled, every `complete` mutation requires a non-empty latest `todo/write` list after the current goal's creation event, with every entry marked `completed`. Missing, stale-from-before-goal, pending, or in-progress lists fail before the goal mutation.

`completionAuditorProvider` independently opts a deployment into semantic review. A completion request starts a fresh one-shot subagent over that provider, gives it only the objective, inherited workspace, and current task list, and requires a schema-validated pass/reject result. Optional paired model-provider and model fields route the auditor independently from the executor; positive integer fields bound its output tokens, attempts per executor turn, and returned feedback characters. The child inherits delegated sandbox and approval policy, receives a source-modification prohibition as its persona, and loses first-party edit, goal, todo, delegation, workflow, and Code Mode tools. It may retain shell tools so it can execute tests and builds.

The goal remains active while review runs. A rejection returns the structured findings through the failed `update_goal` result, so the executor receives them in the next model step and can correct the work without a separate scheduler. Invalid output, an unavailable provider, abnormal child termination, PASS with findings, and audit infrastructure failure all fail closed. Only PASS with no findings reaches the existing compare-and-set completion mutation. A per-parent-turn counter prevents repeated completion calls from starting unbounded audits; the next Goal Round receives a fresh allowance.

## Testing

Unit coverage pins registration and disposal, exclusive scheduling, generated prompt policy, filler-safe generic presentation, direct-human creation in a non-English turn, exact/stale/non-running agent and driver checks, live-child rejection, resumed-fork root authority, steering, mismatched initiators, read/create/partial-edit/pause/resume behavior including strict-schema fillers, conditional blocker explanations, rearming after a session-start edge, authority-before-conditional-argument failures, exact goal-round completion, missing/incomplete/completed todo evidence, auditor model routing and tool restriction, rejection feedback, corrected retry, invalid-verdict failure, attempt bounds, autonomous-only terminal stopping, the configured blocking threshold, and immediate human blocking. A real Loader composition additionally proves that a cordis.yml-selected structured auditor runs before the goal mutation. A keyless replay snapshot mounts the goal domain and tools into the real headless one-shot application, drives a strict-filler `update_goal` probe plus `create_goal` and `get_goal` through the shipped loop and persistence stack, pins its stream-json transcript, and inspects the externally persisted goal change.
...
- **Let the executor approve its own evidence** — rejected because the same context that produced a defect is likely to repeat its assumptions; a fresh child receives the objective and inspects current artifacts independently.
- **Mark complete and reopen after rejection** — rejected because terminal state must never be visible before verification passes, and reopening would add an unnecessary durable lifecycle transition.
- **Disable every shell tool for the auditor** — rejected because project tests, builds, syntax checks, and local HTTP probes require an execution tool. The child loses direct mutation tools and is instructed not to change source, while shell-produced caches and build artifacts remain an accepted cost of validation.
...
- Deployments can require an independently routed, bounded completion review without changing the goal service or agent loop.
- Review failures remain ordinary model-visible tool failures, so the existing goal driver continues correction work and terminal state never appears prematurely.
- Strict-schema provider fillers interoperate without allowing meaningful cross-action updates.

## Known limitations and deferred work

- Semantic classification of a substantial goal, a request to continue, and the same blocking condition remains executor judgment. Completion review remains auditor model judgment rather than a cryptographic certificate.
- An auditor retaining `bash` or `pwsh` is not filesystem-read-only: test scripts may create caches and build artifacts even though direct mutation tools are removed and the auditor persona forbids source changes.
