...
The continuation mechanism has no duplicate goal or blocking tunables: `maxGoalRounds` is resolved and persisted by `dsh-goal`, and the same-condition blocking threshold is resolved and prompted by `dsh-tool-goal`. The driver does own an opt-in admission policy, `autoStartPresets` plus `autoStartMaxGoalRounds`, for deployments that want accepted direct-human implementation work to become a goal even when the model does not call `create_goal`. The deterministic classifier requires imperative execution language plus a concrete work target, persistence language, or substantial structure; response-only health probes stay single-turn. Admission occurs only after downstream `agent/pre-step` listeners accept the human batch, only for live root agents in a named preset, and failure to create never rejects the human step.
...
The unit suite uses the real agent loop and session service with only the model scripted. It covers exact sequential admission and cap enforcement, preset-scoped automatic admission and classifier exclusions, load/resume inertness, every outcome classification, rate limiting, request errors, max tokens, downstream prompt veto, pre-admission and in-flight cancellation, unrelated-human cancellation, failed-pause fallback, human-input ordering, queued and downstream revision races, forged goal attribution, failed mutation and turn checkpoints including a later one-shot injection, scheduler and custom-agent failures, session-start reset, exact lifecycle retirement, and queued/running plugin teardown.
