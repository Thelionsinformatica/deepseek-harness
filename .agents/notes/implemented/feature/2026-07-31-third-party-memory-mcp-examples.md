...
| Apply an exact raw-name allowlist and reconnect a lost transport within a bounded budget | Yes | Keep the provider service reachable and compatible |
| Account, auth, model, embedding, storage initialization | No | Yes |
| Vendor data migration and provider-service recovery | No | Yes |
...
Storage remains provider-owned. Memorix uses `~/.memorix/data` and Engram uses `~/.engram` by default. The Reference Memory example sets a stable `$HOME/.dsh-mcp-reference-memory.jsonl` path instead of writing into the installed npm package directory. Letta owns its database, memory blocks, archives, embeddings, and migrations behind the configured `LETTA_BASE_URL`; its overlay passes only that URL and `LETTA_PASSWORD` to the scrubbed bridge process and never forwards DSH model credentials. Each provider's own environment variable can override these locations before DSH starts.
...
Remote CI never contacts third-party services or consumes secrets. The keyless suite parses all four overlay files, checks their generic bridge, allowlist, and secret boundary, replaces the upstream endpoint with the package-owned MCP fixture server, boots the real Cordis Loader, and proves tool discovery.
...
"Fresh session" means a new DSH session in the same Host. No Host restart is required. The generic MCP client discovers asynchronously and reconnects a lost transport with bounded exponential backoff. Validation waits for tools before the first turn; HMR or a Host restart is required only after the reconnect budget is exhausted.
...
**Expose every Letta MCP Server tool.** Rejected because agent, tool, source, job, file, and MCP administration are not memory operations, enlarge the model's authority, and add schema tokens to every request. The Letta overlay admits the one consolidated memory tool by exact raw name.

## Consequences

Selecting a file gives the model the provider's discovered MCP tools after any configured allowlist, with schema/token cost determined by that admitted set. Removing `--patch` removes the memory server. Users accept each upstream license, data policy, cloud cost, and operational model directly.
