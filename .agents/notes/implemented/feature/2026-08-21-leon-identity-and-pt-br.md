...
The official browser-brand plugin occupies the existing sidebar and conversation brand slots with an original lion SVG, the The Lions Informática wordmark, and Leon as the highlighted product name. The components use the shared Web theme tokens, so the mark and wordmark remain legible in light, dark, and system themes without a second theme service or global stylesheet.
...
Locale tests pin the `pt` selector entry, Brazilian Portuguese provisional default, `pt-BR` document language, explicit Host persistence, English dictionary fallback, central translation pack, and interpolation behavior. Brand tests pin the accessible Leon label, wordmark text, requested mark dimensions, profile gating, declaration order, HMR collapse, and teardown.

## Consequences

Leon starts in Brazilian Portuguese and carries The Lions Informática identity through the plugin composition points the upstream application already owns. Upstream synchronization keeps stable package addresses and a readable English safety path. The cost is maintaining the central Portuguese table for feature copy that has not moved to a native dictionary; inline strings outside the locale service remain a separate migration task.
