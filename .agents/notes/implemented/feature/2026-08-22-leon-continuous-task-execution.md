...
Automatic Goal Rounds retain the same `todo_write` projection. At every continuation boundary, their prompt requires the model to reconcile that plan with inspected workspace artifacts and the latest failed or missing verification before writing, preserve every content string and relative order while updating statuses and retaining or inserting legitimate discoveries, and continue from the first unfinished item instead of recreating earlier work. It accepts only actual tool results as command evidence and requires requested runtime checks to run again after the final change. Leon additionally configures `todo_write` with `preserveExistingItems: true`: later writes may insert discovered work and update statuses, but cannot remove, rename, reorder, or reopen existing completed items. The preservation rule starts the compacted tool description, and every preservation rejection returns the complete accepted list so the model can retry without reconstructing it from compacted history. A later direct-human message starts a fresh plan only when no unfinished durable goal owns the checklist; a resume after restart retains that goal-owned plan, while creation of a different goal clears the previous goal's visible checklist. Leon's `tool-goal` composition enables `completionRequiresCompletedTodos` and a fresh `spawn` completion auditor routed to Google Gemini. A premature completion rejection returns the complete goal-scoped list, including completed entries; a completion mutation therefore requires an all-completed non-empty task list plus an independent structured PASS with no findings, and rejection never publishes terminal state. If the configured Round limit is reached, the Goal bar shows the localized limit visibly before the objective and remains blocked; a visible `Continue +20` action increases the cap and resumes with the edit acknowledgement's fresh revision. Portuguese todo counts also remain localized instead of falling back to English fragments.
