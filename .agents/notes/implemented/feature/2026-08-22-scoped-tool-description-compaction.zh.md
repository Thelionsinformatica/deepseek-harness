...
Leon 使用 Native 呈现、44 字符描述上限、8 KiB workspace 指令预算和 120 字符 skill 目录摘要。它的真实 Web 组合把静态系统段限制为 13,000 字节、工具 schema 限制为 22,000 字节，两者序列化后的合计限制为 31,200 字节，从 32,000 字节的启动预算中至少保留 800 字节。Leon persona、pwsh 策略、workspace 与个人记忆策略以及默认个性化中的重复措辞会单独精简，同时保留测试固定的安全短语和完整注册工具定义。

## 验证

作用域注册表测试覆盖祖先继承、近端覆盖、dispose、递归参数描述压缩、无效声明、重复声明以及注册定义保持完整。呈现行快照固定其配置与面向模型的输出。随附 Web 组合测试会启动真实 Leon preset，并测量 10,020 字节的静态段和 48 个工具 schema 的 21,132 字节，合计 31,152 字节，启动保留容量为 848 字节。此前 31,666 字节的组合只剩 334 字节。
