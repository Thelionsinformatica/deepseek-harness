...
本基础增量不提供经过身份验证的多用户所有权、静态加密、个人语义检索或通用 secret 检测器。本地文件继承已配置存储后端与操作系统保护。后续的[个人记忆用户控制 Note](2026-08-25-leon-personal-memory-control.zh.md)在不合并个人与项目记录的前提下增加了查询、纠正、删除和禁用；导出与备份控制仍属于后续工作。
