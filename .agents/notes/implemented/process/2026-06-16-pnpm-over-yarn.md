...
- **Build-script allowlist.** pnpm 10+ does not run dependency lifecycle scripts unless allowlisted. `pnpm-workspace.yaml` carries an explicit `allowBuilds` map and denies every unreviewed lifecycle script — the same supply-chain-hardening posture the repo already takes toward model/tool output, now applied to install-time code execution. `peerDependencyRules.allowedVersions.typescript: '>=5 <7'` silences benign peer-range warnings for the in-repo TypeScript.
- **Advisory-scoped resolution overrides.** `pnpm-workspace.yaml` pins patched transitive versions when an upstream dependency cannot yet select a fixed release. Each override is limited to the vulnerable range: the filesystem MCP's `brace-expansion` 2.x resolves to 2.1.4, and affected Vite 5.x/6.x versions resolve to 6.4.3 because earlier releases lack the Windows `server.fs.deny` correction, including VitePress 1.6.4's declared Vite 5 dependency. Vitest 4.1.8 remains paired with Vite 8.0.16 because its module runner must execute the repository's supported `import.meta.resolve` paths; the security override does not downgrade that test-only peer. Every cross-major override requires a frozen install, the affected consumer's tests and build, and both production and complete `pnpm audit` checks; remove it once upstream admits a secure version.
- **Patched documentation renderer.** The direct Mermaid dependency is pinned to 11.16.1 in both the root and documentation workspace, while the vulnerable DOMPurify range is overridden to 3.4.13. The complete dependency audit is therefore clean rather than accepting documentation-only CSS injection, prototype pollution, XSS, or denial-of-service advisories as residual debt.
...
- **Wait for upstream manifests before fixing transitive advisories** — leaves a known high-severity dependency reachable even when a compatible patched release has been verified locally.
- **Run `pnpm audit --fix` or broadly upgrade the affected ecosystems** — expands the lockfile and compatibility surface beyond the advisory; scoped overrides keep the decision reviewable and reversible.
...
Security overrides create a manual retirement duty and may deliberately cross an upstream semver range; the affected consumer's build and focused tests are the compatibility proof. In exchange, high-severity advisories do not remain reachable while the upstream manifest catches up.
