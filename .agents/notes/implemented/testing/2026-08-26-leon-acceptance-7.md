...
The local-execution criterion enables the real loopback Ollama profile on every repetition. It verifies 30 fixed PT-BR microtask answers through the exact automatic `qwen3.5:9b` id, for 90 minimum local inferences, while the routing sentinel rejects an external provider before adapter dispatch. Manual catalog models are not automatic-route prerequisites. The complete product decision runs on Windows because its exact-window UI Automation evidence is native and skipped elsewhere.
