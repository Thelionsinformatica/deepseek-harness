...
`dsh doctor` is a boot-free, read-only launcher mode. It checks the current Node version against `^22.19 || >=24`, requires `pwsh` on Windows, inspects read/write access to the resolved Harness home and default workspace, verifies the selected profile's `package.json` and `cordis.patch.yml` when the profile exists, and detects the built `lib/bin.js`. A missing home, uninitialized profile, stopped Web process, missing build artifact during source execution, or unavailable Ollama endpoint is a warning; an unsupported runtime, inaccessible workspace, incomplete installed profile, missing Windows PowerShell, or a different service answering on an expected endpoint is a failure.

The Ollama probe sends only `GET <OLLAMA_HOST-or-loopback>/api/tags`, caps the retained response prefix, and reports whether `qwen3.5:9b` is present for Leon Automatic. Manually selectable Ornith and Qwen 3.8 models do not affect automatic-route readiness. External providers (Google, OpenAI, NVIDIA NIM, OpenRouter) have no dedicated probe: they are credential-referenced routes, not locally started services. The Web probe sends one bounded GET to `http://127.0.0.1:<port>/` and identifies Leon from the returned page title or The Lions branding. None of these requests sends prompts or credentials. The command never creates a path, initializes a profile, starts or stops a service, changes a port, or attempts a repair.
