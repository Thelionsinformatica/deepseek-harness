...
const ctx = await boot('leon-project-engineer-snapshot', rootConfigPath, [
  ...loadOverlayPatches('leon-project-engineer-snapshot', basePatchPath),
  ...loadOverlayPatches('leon-project-engineer-snapshot', overlayPath),
])

try {
  const agentId = SessionId('leon-project-engineer-snapshot')
  ...
  const agent: Agent = {
    ...
    followup: () => {},
    steer: () => {},
    inject: () => { throw new Error('Leon project engineer snapshot must receive the catalog at the step boundary') },
    cancel: () => {},
