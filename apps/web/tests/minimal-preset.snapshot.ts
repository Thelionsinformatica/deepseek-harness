...
describe('minimal agent preset', () => {
  ...
  it('sends the exact RL prompt and schemas, then executes the persistent shell and editor', async () => {
    ...
    await scaffold.ctx.tools.execute({
      ...
      arguments: {
        command: process.platform === 'win32'
          ? `Set-Location -LiteralPath ${pwshLiteral(stateDir)}; $env:DSH_MINIMAL_STATE = 'PERSISTED'`
    ...
    const normalizedCwd = scaffold.workspaceCwd.replaceAll('\\', '/')
    ...
    expect({
      ...
      editor: [
        "Here's the content of {{cwd}}/preset-smoke.txt with line numbers (which has a total of 2 lines):",
        '     1  MINIMAL_EDITOR_OK',
        '     2',
      ].join('\n'),
