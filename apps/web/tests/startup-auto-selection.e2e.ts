...
describe('web e2e: startup auto-selection', () => {
  ...
  it('keeps the resident Hero and composer nodes when the first Workspace session appears', async () => {
    ...
    const leonMark = page.getByRole('img', { name: 'The Lions Informática — Leon' }).last()
    await leonMark.waitFor({ timeout: 10_000 })
    const leonIcon = leonMark.locator('img')
    expect(await leonIcon.getAttribute('src')).toBe('/favicon.ico')
    expect(await leonIcon.isVisible()).toBe(true)
    await page.evaluate(() => {
      const refs = {
