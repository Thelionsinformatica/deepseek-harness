...
- 当前计数：记忆范围内有 **9 个 `.spec.ts` 文件**和 **139 个运行时用例**，另有审查面板浏览器组件用例及 LEON-EVAL 协调器测试。
- 已覆盖范围：
...
- 可执行的 [LEON-EVAL-PTBR](../evals/leon-eval-ptbr.zh.md) 验收把 31 个 PT-BR 场景映射到精确的无需密钥证据，重复三次基线，并测量通过率、泄漏场景组、p50/p95 延迟、堆内存和进程 RSS。
- 剩余评估缺口：真实 Ollama 冷启动、GPU 吞吐量与 embedding 质量需要特定部署的硬件运行；确定性套件使用受控 loopback 语义端点。
...
- 已实现：
  ...
  - 工作区隔离，以及纠正与遗忘时的修订检查。
  - LEON-EVAL-PTBR，包含 31 个无内容、无需密钥的场景、三次运行稳定性、关键隔离和隐私路径硬失败以及脱敏本地报告。
- V2 尚未实现：
  - 用于更改自动写入允许列表的设置 UI；随产品提供的 Host 组合仍默认关闭。
  - 整个会话范围内基于精确 tokenizer 的计费；当前自动快照使用确定性的严格字符预算。
  - 用于可选语义索引的特定部署实时模型性能 profile。
...
4. 混合检索、安全上下文组合和时间历史：已实现；语义检索仍保持默认关闭，等待真实硬件验证。
5. 具有客观标准的 LEON-EVAL-PTBR：31 场景可执行套件已完成。
6. 可逆迁移与回滚计划：已记录。
