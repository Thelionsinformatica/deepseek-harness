...
提取候选项的评估器只接收元数据，不接收候选文本。`store` 结果只是记录在本地队列中的建议；它本身不授权调用 `ctx.memory.create()`。最终存储还必须有操作员明确批准，以及精确的用户和 workspace 功能开关。
...
- 每个已审查候选项都会记录带状态、原因和时间戳的 `autoWrite` 追踪。成功写入还会保留最终记忆 ID 与修订号；结果不确定的 `writing` 状态绝不会自动重试。
...
- M2-008 受控持久化已实现，但随产品提供时保持关闭：只有已批准、非敏感、策略为 `store`，并且精确用户与 workspace 均位于允许列表中的候选项，才能调用 `ctx.memory.create()`。
