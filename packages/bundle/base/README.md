...
- **OmniRoute is a separate loopback service** — its configured endpoint is `http://127.0.0.1:20128/v1`; the route sends a non-secret compatibility bearer header required by the OpenAI-compatible adapter. Production startup must keep the service bound to `127.0.0.1` and make its upstream credentials and budget policy explicit.
