...
preset 文件提供一套未国际化的 `name` 与 `description`，Web 将其用于所有 `user` 行和未知的 `system` 行。对于五个随附 id（`standard`、`code`、`minimal`、`cordis` 与 `leon`），只有名单将该行标记为 `system` 时，Web 才会从当前 locale 解析这两个字段；同名的 `user` preset 仍使用其文件元数据。
...
当前默认 preset 位于始终可见的「主要助手」分组，因为新会话实际获得的正是它。其他随附 preset 初始收起在「高级模式」分组内；用户拥有的 preset 与「创造模式」入口则保持独立。卡片操作把图标与可见标签配对，因此查看、打开、复制和删除不依赖图标识别或悬停。
