...
仅当 `DSH_CLIENT_BUILD_PROFILE` 为 `official` 时，本包才以 The Lions Informática 标识和突出显示的 Leon 产品名填充 `sidebar.brand.mark`、`sidebar.brand.name` 和 `conversation.hero.brand.mark`。其他构建仍会加载插件，但不注册 occupant，因此显示 shell fallback。
