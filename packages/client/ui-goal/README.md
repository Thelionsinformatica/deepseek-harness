...
Goal surface plugin, browser half: the `GoalBar` strip is the second standalone card in the `conversation.input.dock` composer-context stack (order 10, after Todo and before Queue). The live goal arrives through `useProjection('goal')` — the host-computed whole value seeded by the history tail page and updated by `session/projection` frames — so the plugin owns no domain store, refresh chain, or event listener. The slot inject face carries edit, pause, resume, round-limit recovery, and clear actions through `ctx.remote.goals`; each reads the current projection at call time and surfaces a rejected Remote error inline. A goal blocked by `round-limit` renders its localized configured limit before the objective instead of hiding the reason in a hover-only tooltip; it remains blocked and never presents completion copy. Its visible `Continue +20` action increases the cap by 20 and resumes with the edit acknowledgement's fresh CAS ref. The strip single-flights mutations synchronously because React's pending render cannot fence same-frame clicks; after a successful clear it immediately suppresses that exact goal id while the authoritative null projection catches up. Goal creation stays on the `/goal` host command; loading, absent, completed, and successfully cleared goals render nothing.
