...
export const zh = {
  ...
  'automatic.description': '根据任务选择已配置的本地路由；只有部署声明了外部备用路由且获得授权后才使用 API',
...
export const en = {
  ...
  'automatic.description': 'Chooses among the configured local routes for each task; it uses an API only when the deployment declares an external fallback and you consent',
...
export const pt = {
  ...
  'automatic.description': 'Escolhe entre as rotas locais configuradas conforme a tarefa. Só usa API quando o perfil oferece fallback externo e você autoriza.',
