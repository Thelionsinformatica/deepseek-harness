...
提供方呈现采用本地优先顺序：活跃且无需引用的路由（包括本地认证或提供方原生认证）显示在其他可用路由和仍需设置的可选路由之前。每组内部保持 Host 顺序稳定。
...
分配面板通过已注册目录和带版本检查的设置写入配置主模型及已启用的辅助消费方。外部分配需要明确确认上下文传输和 API 费用，Host 策略独立检查授权。清除功能恢复消费方继承，不一定使用主模型。刷新目录不执行推理。协作卡配置工作代理与完成审查，不启动或重新配置实验团队；不受支持的独立 MCP、技能和审批路由没有可编辑选择器。
