...
区头始终可发现的**已归档会话**操作会打开带数量的列表，在数量为零时也会显示清晰的空状态，并提供恢复和需确认的永久删除。每个非空白 Session 行也会在重命名、Fork 和 Archive 旁提供需确认的删除会话操作。永久删除的确认会说明：会话历史将被删除，Workspace 文件和单独保留的个人记忆不受影响；请求未完成时对话框保持打开，失败时保留对应行并显示告警。
