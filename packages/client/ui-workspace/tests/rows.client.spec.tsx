...
describe('workspace browser rows', () => {
  ...
  it('session row menu dispatches rename, fork, archive, and confirmed-delete request without opening', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    // Delete delegates the confirmation request with the current display title.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
