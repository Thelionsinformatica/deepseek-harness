...
- **摘要**：直接 `llm/stream` 调用使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 agent（智能体）目标，而不运行仅用于 agent loop 的 `agent/request` 扩展点。该调用会逐字回放会话自身的系统提示词、工具与已遮蔽区域消息（包括图片引用），并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效。所选适配器必须解析或明确拒绝这些图片。它将 `GenerateOptions.purpose` 设为 `compaction`，适配器可将其作为请求归因转发（DeepSeek 适配器发送 `x-deepseek-harness-compact: 1`），但不会触碰模型可见的请求体。只有返回的文本会进入检查点；推理（reasoning）和工具调用都会被排除，以免泄露私有推理或产生遗留调用；图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。随后，后端会添加一个有界的确定性连续性部分，其中包含精确的 HTTP(S) 引用、Windows 路径和选定的工具结果状态行，避免能力较弱的摘要模型静默丢失这些操作事实。
- **框定**：替换 user 消息使用 `<compacted-summary>` 标签标记已建立的检查点上下文。检查点摘要（包括确定性连续性部分）保留在 `compaction/summary` 事件上；未经修改的模型输出保留在 `rawOutput` 中，后续自动周期会合并之前的检查点。
...
成功步骤越过阈值后，如果已加载可选 pruner，超大工具结果会先被改写。如果仍需摘要，下一个请求会收到下方检查点前导、一个空行、`<compacted-summary>`、根据数据生成的摘要、可选的 `Operational Continuity (deterministic)` 部分以及 `</compacted-summary>`。该确定性部分会移除凭据赋值、提供方密钥形式、URL 凭据、查询字符串和片段；它按来源标记引用，并且仅将来自工具结果的状态标记为状态证据。溢出恢复会根据使表层前进的任何替换重建立即重试。检查点会替换已选较早范围，后面跟随已保留的近期单元。
...
不依赖模型的剪枝可以完全避免辅助调用；否则它会在摘要替换较早范围之前缩减该调用的 transcript（文本记录）。确定性连续性部分最多添加十六条引用和六条有界证据行。替换会缩减未来输入历史，而非追加第二份副本。摘要会保留到后续压缩将其替换，但不可分的非工具单元仍可能超出预算。
...
- **确定性连续性有意保持狭窄**：它保留 HTTP(S) 引用、Windows 路径和简短的工具结果状态行；结构化模型摘要仍负责意图、决策、待办工作和下一步操作。保留的状态只是在压缩时的观察，不证明进程或端点在重启后仍然存活。
