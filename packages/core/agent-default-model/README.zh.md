...
可选组合策略 `auxiliaryModels` 启用 `agent-model-roles` 设置命名空间。标题、压缩、视觉、工作代理及审查条目包含提供商、模型、可选推理强度及显式外部授权。消费方在记录请求或创建子代理前解析分配。缺失条目保持现有继承行为；组合未启用时实验配置不变。只有部署声明的 `localProviders` 被视为本地，即使端点使用 localhost 也不例外。其他路由必须设置 `allowExternal: true`。该策略不授予工具权限、费用预算或额外代理。更改功能影响下次调用或新建子代理，不保证跨模型缓存复用。
