...
- `ctx.tools.compactDescriptions(maxLength: number): () => void` normalizes whitespace and caps tool and parameter descriptions in this scope's model-facing Native schemas and Code Mode SDK. It requires an agent scope and an integer of at least `3`; a second declaration in the same scope throws, while a nearer scope may override an ancestor. Registry definitions and `schemas()` remain complete. Disposed with the calling fiber.
