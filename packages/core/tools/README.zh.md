...
- `ctx.tools.compactDescriptions(maxLength: number): () => void`：规范化空白，并限制本 scope 面向模型的 Native schema 与 Code Mode SDK 中工具描述和参数描述的长度。它要求 agent scope 和不小于 `3` 的整数；同一 scope 内的第二次声明会抛出，而更近的 scope 可以覆盖祖先声明。注册表定义与 `schemas()` 保持完整。随调用方 fiber dispose。
