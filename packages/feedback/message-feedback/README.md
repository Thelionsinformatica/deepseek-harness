...
- **Permanent deletion is Host-orchestrated** — internal `purgeSession(sessionId)` removes the complete sidecar row idempotently and serializes with item puts/deletes, but is deliberately not a user Remote. Ordinary `session/disposed` / `host/session-removed` detach still preserves feedback; only the permanent Session lifecycle invokes the purge after canonical log deletion.
