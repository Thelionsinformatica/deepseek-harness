...
- **永久删除由 Host 编排**——内部 `purgeSession(sessionId)` 会幂等删除完整 sidecar 行，并与条目 put／delete 串行，但刻意不作为用户 Remote 暴露。普通 `session/disposed`／`host/session-removed` detach 仍保留反馈；只有永久 Session 生命周期在规范日志删除后调用该清理。
