...
export function renderGoalRoundPrompt(goal: GoalView, round: number): ContentBlock[] {
  return [{
    ...
    text: '<goal_round>\n'
      ...
      + 'broken. Preserve every existing todo content string and its relative order exactly; update statuses and '
      + 'retain or insert legitimately discovered items. Do not replace or rename the plan merely because a new '
      + 'round started. Work from '
      + 'the first unfinished item. Make concrete '
