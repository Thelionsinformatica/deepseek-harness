...
function guidance(
  ...
  return 'Use goal tools only for one long-running objective; skip routine single-turn work. create_goal may infer '
    + 'goal intent from a direct human request in any language. A deployment may create it automatically: call '
    + 'get_goal first, then use exact goal_id/revision. Resuming a session or forking it disarms an active goal; any human continue '
    + 'or resume request in any wording or language requires update_goal action resume. Complete only when achieved. '
    + `Block only after the same condition lasts at least ${blockedAfter} consecutive goal rounds; set blocked_reason. `
