...
| `PreToolUse` | `tools/pre-execute` (waterfall) | `continue:false` or `deny` → `PreToolDecision.deny`; `ask` → `PreToolDecision.ask`; a stop takes precedence and skips the tool body |
...
Provider-supplied reasons pass through verbatim. When absent, a blocked prompt uses exactly `blocked by UserPromptSubmit hook`, a denied tool becomes `Error: blocked by PreToolUse hook`, a `continue:false` pre-tool veto becomes `Error: stopped by PreToolUse hook`, blocked post-tool feedback is exactly `blocked by PostToolUse hook`, and a blocking stop adds steering exactly `continue: blocked by Stop hook`. `systemMessage` and `updatedInput` are logged or warned but are not model-visible in this implementation.
...
- **Common payload and output fields are partial:** mapped event payloads omit `prompt_id`, `transcript_path`, `permission_mode`, and `effort` where Claude Code would provide them. `systemMessage` is logged + warned but not surfaced. `{"continue": false}` vetoes the current call at `PreToolUse` and applies `stopReason` there; other mapped points still only record the stop request because an agent-wide halt primitive is unavailable. `suppressOutput` and `terminalSequence` are not applied (`TODO(hook-continue-false)`).
