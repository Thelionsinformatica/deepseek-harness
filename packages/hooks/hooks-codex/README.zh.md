...
| `PreToolUse` | `tools/pre-execute`（waterfall） | `continue:false` 或 `block` → `PreToolDecision.deny`（没有 `allow`／`ask`）；停止请求优先，并跳过工具主体 |
...
提供方提供的原因逐字传递。缺失原因时，已阻塞提示词精确使用 `blocked by UserPromptSubmit hook`，已拒绝工具变为 `Error: blocked by PreToolUse hook`，`continue:false` 的工具前否决变为 `Error: stopped by PreToolUse hook`，已阻塞工具后反馈精确为 `blocked by PostToolUse hook`，阻塞 stop 则精确添加 steering `continue: blocked by Stop hook`。Codex `systemMessage` 不会呈现。
...
- **`PreToolUse` 只支持部分功能：** 阻塞和 `continue:false` 都会否决当前工具调用；若提供 `stopReason`，则使用该原因。会忽略 `additionalContext`、`permissionDecision: "allow"` 和 `updatedInput`。每个工具都表示为 `tool_input: { command }`，因此非 shell 工具参数不会如实公开给 hook。
...
- **通用 payload 与输出字段只支持部分功能：** 每个已映射事件都报告静态配置的 `model` 与 `permission_mode: "default"`，而非当前 Codex 运行时值。`systemMessage` 会被记录并触发警告，但不呈现。`{"continue": false}` 会在 `PreToolUse` 否决当前调用；其他已映射点仍只记录停止请求，因为尚无 agent 级硬停止原语（`TODO(hook-continue-false)`）。
