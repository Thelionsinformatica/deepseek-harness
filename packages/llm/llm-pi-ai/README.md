...
- An OmniRoute response carrying `x-omniroute-response-cost` adds the validated decimal USD charge to durable usage as integer `providerCostUsdNanos`. Non-streaming responses carry it in an HTTP response header; streaming responses carry it in a terminal SSE comment immediately before `[DONE]`, which a request-scoped fetch observer reads without changing the provider byte stream. Missing, negative, malformed, or unsafe values are omitted; no fixed gateway price is guessed.
