...
- For legacy compatibility only, an OmniRoute response carrying `x-omniroute-response-cost` adds the validated decimal USD charge to durable usage as integer `providerCostUsdNanos`. The header is ignored for every other provider, including FreeLLMAPI. Non-streaming OmniRoute responses carry it in an HTTP response header; streaming responses carry it in a terminal SSE comment immediately before `[DONE]`, which a request-scoped fetch observer reads without changing the provider byte stream. Missing, negative, malformed, or unsafe values are omitted; no fixed gateway price is guessed.
