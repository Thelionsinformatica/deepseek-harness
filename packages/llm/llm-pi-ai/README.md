...
Configure credentials, the model catalog, and deployment-specific transport settings per provider, keyed by the provider route itself. Each profile may set a `retryPolicy`; omission uses normal mode with five retries. `apiKeyEnv` is a credential *reference* resolved per request, so no secret enters this file. `apiKeyEnvFallbacks` is an optional ordered list of compatibility references: the primary wins, then only the declared fallbacks are tried. It requires a primary reference and rejects duplicates. Omitting every credential reference leaves the route unauthenticated, which for an installed catalog route means pi-ai's provider-native ambient discovery; a configured chain that resolves to nothing fails the request with `MISSING_CREDENTIAL` instead, because falling through would authenticate with whatever unrelated key the environment happens to hold. One credential serves every model on its route.
...
- For legacy compatibility only, an OmniRoute response carrying `x-omniroute-response-cost` adds the validated decimal USD charge to durable usage as integer `providerCostUsdNanos`. The header is ignored for every other provider, including OpenRouter. Non-streaming OmniRoute responses carry it in an HTTP response header; streaming responses carry it in a terminal SSE comment immediately before `[DONE]`, which a request-scoped fetch observer reads without changing the provider byte stream. Missing, negative, malformed, or unsafe values are omitted; no fixed gateway price is guessed.
