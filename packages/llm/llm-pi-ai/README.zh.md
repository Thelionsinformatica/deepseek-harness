...
按提供方配置凭据、模型 catalog 与部署特定传输设置，并以提供方路由本身为键。每个 profile 都可以设置 `retryPolicy`；省略时使用 normal 模式并重试五次。`apiKeyEnv` 是按请求解析的凭据*引用*，因此机密不进入该文件。`apiKeyEnvFallbacks` 是可选的有序兼容引用列表：主引用优先，之后只尝试明确声明的 fallback；它要求先配置主引用，并拒绝重复项。完全省略凭据引用会让该路由处于未认证状态；对已安装 catalog 路由而言，这意味着交给 pi-ai 的提供方原生环境发现。已配置却解析不出任何值的引用链则相反，会让请求以 `MISSING_CREDENTIAL` 失败，因为放行下去就会用环境里恰好持有的某个无关密钥完成认证。一条凭据服务该路由下的全部模型。
...
- 仅为保留旧版兼容性，携带 `x-omniroute-response-cost` 的 OmniRoute 响应会把已校验的十进制美元费用以整数 `providerCostUsdNanos` 加入持久 usage。所有其他提供方（包括 OpenRouter）都会忽略该标头。非流式 OmniRoute 响应通过 HTTP 响应标头携带该值；流式响应则在 `[DONE]` 前的终止 SSE 注释中携带，由请求作用域的 fetch 观察器读取且不改变提供方字节流。缺失、负数、格式错误或超出安全整数范围的值会被省略；系统不会猜测固定网关价格。
