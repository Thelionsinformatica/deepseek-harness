...
- 携带 `x-omniroute-response-cost` 的 OmniRoute 响应会把已校验的十进制美元费用以整数 `providerCostUsdNanos` 加入持久 usage。非流式响应通过 HTTP 响应标头携带该值；流式响应则在 `[DONE]` 前的终止 SSE 注释中携带，由请求作用域的 fetch 观察器读取且不改变提供方字节流。缺失、负数、格式错误或超出安全整数范围的值会被省略；系统不会猜测固定网关价格。
