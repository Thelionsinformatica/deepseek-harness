...
The browser-safe types subpath also owns `llm/failover`, the durable provider-replacement notice produced by automatic routing before this plugin's retry policy runs. Its payload records the failed and replacement routes plus the normalized failure; it schedules no delay and does not imply that the retry executor selected either model.

The separately published `./invariant` companion checks that every scheduled retry names the current open turn and latest closed step, matches the failed request's durable provider, carries non-empty provider and policy identities, has mode-specific bounds, a unique step record, the correct provider-policy retry number, and a bounded timer delay. It also requires each `llm/retry-started` event to name one prior scheduled attempt with the same `retryId`, turn, step, and retry number, and rejects repeated started events. A failover event must name the open request step, match its active provider/model, and replace that provider with a different non-empty route. Full jitter may schedule zero milliseconds at its lower boundary.
