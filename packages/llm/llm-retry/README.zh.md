...
可安全用于浏览器的 types 子路径还拥有 `llm/failover`，即自动路由在本插件的重试策略运行前产生的持久提供方替换通知。其载荷记录失败路由、替代路由与规范化失败；它不会安排延迟，也不表示重试执行器选择了任一模型。

单独发布的 `./invariant` 配套模块会检查每个已调度重试是否指向当前开启轮次及其最新已关闭步骤，是否与失败请求的持久提供方匹配，是否携带非空的提供方与策略标识，是否满足 mode 特定边界，是否拥有唯一步骤记录和正确的提供方策略重试编号，以及是否携带有界定时器延迟。它还要求每个 `llm/retry-started` 事件通过相同的 `retryId`、轮次、步骤与重试编号指向一个先前调度的尝试，并拒绝重复的 started 事件。failover 事件必须指向开启的请求步骤，与其活动提供方／模型匹配，并用另一个非空提供方路由替换它。full jitter 可以在下界调度为零毫秒。
