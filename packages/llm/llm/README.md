...
`TokenUsage` keeps disjoint provider token counts and may carry `providerCostUsdNanos` when the provider or gateway reports the exact charge for that request. The value is integer billionths of one US dollar; adapters never synthesize it from a pricing table.
