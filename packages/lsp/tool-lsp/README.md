...
Namespace plugin (`name` / `inject` / `Config` / `apply`, no default export). Injects `tools` and `systemPrompt`; its tool and guidance activate only while an `lsp` service is available.
