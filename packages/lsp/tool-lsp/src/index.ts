 ...
 * `dsh-tool-call-timeout-policy` to enforce. It runtime-injects `tools` and `systemPrompt`, then
 * activates its model-facing surface only while an `lsp` service is available. It imports no provider.
 *
...
/** Always-required services; the model-facing surface activates through a child `lsp` injection. */
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.inject(['lsp'], (lspCtx: Context) => {
    lspCtx.systemPrompt.section({ name: 'tool:lsp', order: 112, text: LSP_PROMPT_TEXT })

    lspCtx.tools.register(defineTool({
      name: 'lsp',
      description:
      ...
      async execute(args, exec) {
        ...
        const result = await lspCtx.lsp.query({
