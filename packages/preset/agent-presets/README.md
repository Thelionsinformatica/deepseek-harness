...
- **Windows copies inherit the writable root's DACL** — Node's POSIX mode bits neither express nor inspect a Windows owner-only ACL. A deployment that requires per-user confidentiality must restrict the configured user-preset root with a native Windows ACL; `copy()` does not harden that DACL.
