...
在 POSIX 上，复制出的目录树被收紧为仅属主可用（文件 `0o600` 并保留属主执行位，目录 `0o700`）。符号链接被解引用以保证副本自包含，且根目录在首次复制时创建——部署配置了尚不存在的用户根目录，正是首次运行的正常状态。复制出的 `preset.yml` 会被重写：保留来源的描述供作者就地编辑，但丢弃其名称与 roster `order`——副本若与来源呈现得一模一样、或按随附集合声明的顺序排序，roster 就不再能区分它们。`remove()` 拒绝随部署提供的 preset；随附集合正是副本的已知良好起点。
...
- **Windows 副本继承可写根目录的 DACL** —— Node 的 POSIX mode bit 既不能表达也不能检查 Windows 的仅属主 ACL。需要单用户保密性的部署必须用原生 Windows ACL 限制所配置的用户 preset 根目录；`copy()` 不会加固该 DACL。
