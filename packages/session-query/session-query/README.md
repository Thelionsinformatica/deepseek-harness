...
- `purgeSession(sessionId)` is the Host-internal permanent-deletion hook for provider-owned derived data. The provider-independent engine stores no index and therefore defaults to an idempotent no-op; materializing backends override it and serialize the purge with reconciliation.
