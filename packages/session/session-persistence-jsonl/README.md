...
- **No retention scheduler** — `delete(id)` validates the stored header and removes only that Session's fixed transcript file; it leaves the containing session/project directories and every user file untouched. Age- or quota-based pruning remains deployment policy.
