...
| `delete(id): Promise<void>` | Permanently remove the backend-owned log for one quiescent Session. First-party coordinator backends fence that id before the serialized delete, reject live or reserved owners, make an absent id an idempotent success, and prevent queued work from recreating the artifact. The cwd and user files are outside this capability. Backends that do not implement deletion reject explicitly. |
...
| `deleteStored?(id)` | Permanently remove only the exact backend-owned identity. Missing ids are successful no-ops. Omission makes the service reject permanent deletion explicitly. |
...
- **No retention or shared-artifact garbage collector** — permanent deletion removes the canonical Session log, while retention schedules and reference-aware cleanup for artifacts shared by forks remain deployment policy.
