...
- `estimatedApiCostUsdNanos` 以 1 美元的十亿分之一为整数存储。提供方或网关上报的精确 `providerCostUsdNanos` 优先；否则通过已配置的提供方/模型价格表为 usage 定价。同一步的流式 usage 样本会由最终消息样本替换，而不是重复计费。`pricedModelCalls` 包含明确的零费用及已配置为零成本的本地路由；`unpricedModelCalls` 会明确暴露未知路由的 usage，避免把不完整估算静默呈现为完整结果。
...
价格表由部署方所有，因为提供方价格、付费/免费方案、货币、生效日期与模型标识都会独立于本包变化。省略 `prices` 会停用基于表格的估算，但仍会折叠存在的提供方精确上报费用。Leon Web 的随附组合采用可编辑的付费 Standard 价格快照；显示总额可同时包含精确网关费用与 token 价格估算，并非提供方账单。
...
- **仅统计持久 usage 中的费用**——提供方/网关响应的精确费用可以包含该响应上报的项目，而表格定价路由仍只估算 token。Search/Maps grounding 请求、缓存存储时长、按时长计价的媒体、税费、抵扣、免费额度、Batch/Flex/Priority 倍率，以及失败且未上报 usage 的调用仍可能不在本 projection 中。UI 会暴露未定价模型调用，但完整账单核对仍需提供方账单控制台。
