...
在 Windows 上，原子替换对 `EACCES`、`EBUSY` 和 `EPERM` 最多重试八次，并保留同一份完整的临时文件。其他错误立即失败；重试耗尽时保留旧目标并删除临时文件。这不提供崩溃持久性。参见[重试决策](../../../.agents/notes/implemented/bug-fix/2026-08-29-windows-atomic-replace-retry.zh.md)。
