...
- `ctx.workspaceRegistry.unarchiveSession(id)` — removes an id from the archive set without moving its existing workspace slot or touching persistence. Repeating an already-restored id is a no-op.
- `ctx.workspaceRegistry.deleteSession(id)` — clears one already-authorized permanent deletion from every workspace account, the archive set, and registry header/path caches. A recoverable pending-mutation marker makes the multi-record cleanup restart-safe. The canonical log is deleted by Session persistence; this registry method never touches the cwd or user files.
...
- Destructive folder removal remains absent and separate; Workspace registration deletion never substitutes for permanent Session deletion, whose Host lifecycle explicitly coordinates persistence, derived records, and this registry cleanup ([decision](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)).
